...
class Controller_Admin_Retrainingcollege extends Controller_Admin {
  private $page_title = 'Курсы переподготовки для колледжей';
  ...
  {
    $retraining = View::factory('admin/v_retraining_college_list');
  ...
  public function action_edit()
  {
    ...
    $retraining = View::factory('admin/v_retraining_college');
    ...
    if ($id)
    {
      $retraining->retraining = ORM::factory('retrainingcollege', $id);
    ...
    {
      $retraining->retraining = ORM::factory('retrainingcollege');
    }
  ...
  public function action_uploaddoc()
  {
    ...
    $dir = substr(ORM::factory('setting', array('key' => 'dir_docs_retraining_college'))->value, 1);
  ...
  public function action_save()
  {
    ...
    if ($id > 0)
    {
      $retraining = ORM::factory('retrainingcollege', $id);
      if (isset($_POST['delete']))
      {
        $this->delete_doc($retraining->file_program);
        $retraining->delete();
        
        $this->request->redirect('admin/retrainingcollege');
      }
    }
    else
    {
      $retraining = ORM::factory('retrainingcollege');
    }
