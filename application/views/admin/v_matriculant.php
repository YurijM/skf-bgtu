...
<div class="admin-list">
	...
	<div class="remark bg-success text-success">
		Для того, чтобы на сайте в разделе <b>Абитуриенту - Поданные заявления</b> отобразились актуальные данные о
		поданных заявлениях, необходимо ввести данные по
	...
	<div class="form-group">
		...
		<select name="section" class="form-control placeholder">
			...
			<? foreach ($sections as $section): ?>
				<option <?= ($matriculant->section_id == $section->id ? 'selected' : '') ?>
					value="<?= $section->id ?>"><?= $education_types[$section->direction->education] . ' - ' . ($section->short ?: $section->section) ?></option>
			<? endforeach ?>
		</select>
	</div>
	...
	<div class="form-group">
		<?= Form::label('condition', 'Условия приёма') ?>
		<?= Form::select(
			'condition',
			$conditions,
			$matriculant->admission_conditions,
			['class' => 'form-control', 'placeholder' => 'Поле admission_conditions']
		) ?>
	</div>

	<div class="form-group col-xs-12">
		<div class="col-xs-12">
			<?= Form::label('consent', 'Согласие получено') ?>
		</div>
