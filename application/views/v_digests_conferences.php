	...
	<? foreach ($conferences as $conference): ?>
		<div class="<?= ($mode == 'normal' ? 'col-md-3 col-sm-4 col-xs-6' : 'col-sm-3 col-xs-6') ?>">
			<div class="thumbnail text-center">
				<?= HTML::anchor(
					($conference->doc_file != '' ? $dir_docs_conferences . $conference->doc_file : $dir_docs . 'empty-doc.pdf'),
					HTML::image($dir_img_conferences . $conference->img_file,
						['class' => 'img img-rounded', 'alt' => $conference->conference]),
					['title' => 'Перечень докладов', 'target' => '_blank']
				) ?>
