<div class="specialties">
	<h2 class="text-center"><?= $page_title ?></h2>

	<h4 id="item1" class="text-center">
		Сведения о количестве мест для приема граждан на обучение по образовательным программам высшего образования -
		программам бакалавриата
	</h4>

	<div class="table-responsive">
		<table class="table table-bordered table-condensed">
			<tr>
				<th rowspan="3">Форма обучения</th>
				<th rowspan="3">Код и наименование специальности или направления подготовки</th>
				<th colspan="7">Количество мест для приема граждан на обучение</th>
			</tr>
			<tr>
				<th colspan="2">за счет бюджетных ассигнований федерального бюджета</th>
				<th colspan="2">за счет бюджетных ассигнований бюджета субъекта Российской Федерации</th>
				<th colspan="2">за счет бюджетных ассигнований местного бюджета</th>
				<th rowspan="2">по договорам об оказании платных образовательных услуг</th>
			</tr>
			<tr>
				<th>всего</th>
				<th>в том числе квота лиц, имеющих особое право</th>
				<th>всего</th>
				<th>в том числе квота лиц, имеющих особое право</th>
				<th>всего</th>
				<th>квота лиц, имеющих особое право</th>
			</tr>
			<tr>
				<th>1</th>
				<th>2</th>
				<th>3</th>
				<th>4</th>
				<th>5</th>
				<th>6</th>
				<th>7</th>
				<th>8</th>
				<th>9</th>
			</tr>
			<tr class="bg-info text-info">
				<td rowspan="3" style="vertical-align: middle" class="text-center">Очная</td>
				<td class="text-center">08.03.01 Строительство</td>
				<td class="text-center" style="vertical-align: middle">10</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">40</td>
			</tr>
			<tr class="bg-info text-info">
				<td class="text-center">09.03.01 Информатика и вычислительная техника</td>
				<td class="text-center" style="vertical-align: middle">5</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">20</td>
			</tr>
			<tr class="bg-info text-info">
				<td class="text-center">38.03.01 Экономика</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">20</td>
			</tr>
			<tr>
				<td rowspan="3" style="vertical-align: middle" class="text-center">Заочная</td>
				<td class="text-center">08.03.01 Строительство</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">100</td>
			</tr>
			<tr>
				<td class="text-center">09.03.01 Информатика и вычислительная техника</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">30</td>
			</tr>
			<tr>
				<td class="text-center">38.03.01 Экономика</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">0</td>
				<td class="text-center" style="vertical-align: middle">30</td>
			</tr>
		</table>
	</div>

	<hr>

	<h4 id="item2" class="text-center">
		Перечень вступительных испытаний, необходимых для поступления
	</h4>

	<div class="table-responsive">
		<table class="table table-bordered table-condensed">
			<tr>
				<th rowspan="2">Шифр</th>
				<th rowspan="2">Направления подготовки</th>
				<th rowspan="2">Приоритетность вступительных испытаний при ранжировании</th>
				<th colspan="3">Форма проведения экзамена</th>
			</tr>
			<tr>
				<th>для выпускников школ</th>
				<th>для выпускников на базе профессионального образования</th>
				<th>для лиц, имеющих высшее образование</th>
			</tr>
			<tr>
				<td>08.03.01</td>
				<td>Строительство</td>
				<td>
					<ul type="disc">
						<li><u>математика</u><sup>1</sup>;</li>
						<li>физика;</li>
						<li>русский язык</li>
					</ul>
				</td>
				<td class="text-center">ЕГЭ</td>
				<td class="text-center">письменно</td>
				<td class="text-center">тестирование</td>
			</tr>
			<tr>
				<td>09.03.01</td>
				<td>Информатика и вычислительная техника</td>
				<td>
					<ul type="disc">
						<li><u>математика</u><sup>1</sup>;</li>
						<li>информатика;</li>
						<li>русский язык</li>
					</ul>
				</td>
				<td class="text-center">ЕГЭ</td>
				<td class="text-center">письменно</td>
				<td class="text-center">тестирование</td>
			</tr>
			<tr>
				<td>38.03.01</td>
				<td>Экономика</td>
				<td>
					<ul type="disc">
						<li><u>математика</u><sup>1</sup>;</li>
						<li>обществознание;</li>
						<li>русский язык</li>
					</ul>
				</td>
				<td class="text-center">ЕГЭ</td>
				<td class="text-center">письменно</td>
				<td class="text-center">тестирование</td>
			</tr>
		</table>
	</div>
	<div class="remark">
		Примечание:<br>
		<sup>1</sup> - профильный общеобразовательный предмет (обозначен первым и выделен подчеркиванием).
	</div>

	<hr>

	<h4 id="item4" class="text-center">Минимальное количество баллов для каждого вступительного испытания по каждому
		конкурсу</h4>

	<div class="table-responsive">
		<table class="table table-bordered table-condensed">
			<tr>
				<th>Шифр</th>
				<th>Направления подготовки</th>
				<th>1 предмет</th>
				<th>2 предмет (профильный)</th>
				<th>3 предмет</th>
			</tr>
			<tr>
				<td>08.03.01</td>
				<td>Строительство</td>
				<td>Русский язык - <strong>40</strong> баллов</td>
				<td>Математика - <strong>39</strong> баллов</td>
				<td>Физика - <strong>40</strong> баллов</td>
			</tr>
			<tr>
				<td>09.03.01</td>
				<td>Информатика и вычислительная техника</td>
				<td>Русский язык - <strong>40</strong> баллов</td>
				<td>Математика - <strong>39</strong> баллов</td>
				<td>Информатика - <strong>42</strong> баллов</td>
			</tr>
			<tr>
				<td>38.03.01</td>
				<td>Экономика</td>
				<td>Русский язык - <strong>40</strong> баллов</td>
				<td>Математика - <strong>39</strong> баллов</td>
				<td>Обществознание - <strong>44</strong> баллов</td>
			</tr>
		</table>
	</div>

	<div class="col-xs-12">
		<h4 class="text-center">Подача документов онлайн</h4>

		<ol>
			<li style="margin-bottom: 5px">
				Проходим <a href="https://abiturient.bstu.ru/entrant/register" target="_blank">регистрацию</a> в личном
				кабинете абитуриента БГТУ им.В.Г.Шухова, даём согласие на обработку персональных данных. После
				регистрации попадаем в личный кабинет
			</li>
			<li style="margin-bottom: 5px">
				Выбираем вкладку "Заявление". При создании заявления в первой верхней вкладке "Университет" выбираем
				"Северо-Кавказский филиал"
			</li>
			<li style="margin-bottom: 5px">
				Заполняем персональную информацию
			</li>
			<li style="margin-bottom: 5px">
				Добавляем скан-копию паспорта
			</li>
			<li style="margin-bottom: 5px">
				Вносим сведения об образовании, добавляем скан аттестата или диплома
			</li>
			<li style="margin-bottom: 5px">
				Заполняем раздел "Особые права/льготы" (при наличии)
			</li>
			<li style="margin-bottom: 5px">
				Вносим результаты ЕГЭ/Олимпиады (при наличии)
			</li>
			<li style="margin-bottom: 5px">
				Заполняем индивидуальные достижения (при наличии)
			</li>
			<li style="margin-bottom: 5px">
				Добавляем номер и скан-копию СНИЛСа в разделе Дополнительные документы
			</li>
			<li style="margin-bottom: 5px">
				Заполняем платежную информацию для желающих поступать на платную форму обучения
			</li>
			<li style="margin-bottom: 5px">
				Нажимаем «Создать заявление»
			</li>
			<li style="margin-bottom: 5px">
				В личном кабинете размещаем скан-копию согласия на зачисление.
			</li>
		</ol>
	</div>

	<div>
		<b><?= HTML::anchor('/cost', 'Информация о стоимости обучения и оплате') ?></b>
	</div>

	<hr>

	<h4 id="item5" class="text-center">Программы вступительных испытаний</h4>

	<ul type="disc">
		<li>
			<?= HTML::anchor($dir_docs_programs . 'math.pdf', 'программа вступительных испытаний по математике', array('target' => '_blank')) ?>;
		</li>
		<li>
			<?= HTML::anchor($dir_docs_programs . 'math_ticket.pdf', 'образец экзаменационного билета по математике', array('target' => '_blank')) ?>;
		</li>
		<li>
			<?= HTML::anchor($dir_docs_programs . 'rus.pdf', 'программа вступительных испытаний по русскому языку', array('target' => '_blank')) ?>;
		</li>
		<li>
			<?= HTML::anchor($dir_docs_programs . 'rus_ticket.pdf', 'образец экзаменационного билета по русскому языку', array('target' => '_blank')) ?>;
		</li>
		<li>
			<?= HTML::anchor($dir_docs_programs . 'phy.pdf', 'программа вступительных испытаний по физике', array('target' => '_blank')) ?>;
		</li>
		<li>
			<?= HTML::anchor($dir_docs_programs . 'phy_ticket.pdf', 'образец экзаменационного билета по физике', array('target' => '_blank')) ?>;
		</li>
		<li>
			<?= HTML::anchor($dir_docs_programs . 'inf.pdf', 'программа вступительных испытаний по информатике', array('target' => '_blank')) ?>;
		</li>
		<li>
			<?= HTML::anchor($dir_docs_programs . 'inf_ticket.pdf', 'образец экзаменационного билета по информатике',
				array('target' => '_blank')) ?>;
		</li>
		<li>
			<?= HTML::anchor($dir_docs_programs . 'obsch.pdf', 'программа вступительных испытаний по обществознанию', array('target' => '_blank')) ?>;
		</li>
		<li>
			<?= HTML::anchor($dir_docs_programs . 'obsch_ticket.pdf', 'образец экзаменационного билета по обществознанию', array('target' => '_blank'))	?>;
		</li>
	</ul>

	<hr>

	<p id="item6">
		С <strong>дополнительной информацией</strong> можно ознакомиться
		<strong><?= HTML::anchor($link_to_bgtu, 'на сайте головного вуза', array('target' => '_blank')) ?></strong>.
	</p>
</div>

<script>
	$(document).ready(function () {
		$('table:even').addClass('bg-success').addClass('text-success');
		$('table:odd').addClass('bg-info').addClass('text-info');
	});
</script>